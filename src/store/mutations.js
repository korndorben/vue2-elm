...
export default {
  ...
  [RECORD_SUPPLIER](state, detail) {
    state.supplier = detail;
  },
  // 加入购物车
  [ADD_CART](state, {supplierid, categoryid, dish, attr}) {
    let cart = state.cartList; //购物车
    let shop = cart[supplierid] = (cart[supplierid] || {}); //商家
  ...
  // 移出购物车
  [REDUCE_CART](state, {supplierid, categoryid, dish, attr}) {
    let cart = state.cartList; //购物车
    let shop = cart[supplierid] = (cart[supplierid] || {}); //商家
    let category = shop[categoryid] = (shop[categoryid] || {}); //分类
    let dishcontainer = category[dish.id] = (category[dish.id] || {}); //菜品
  ...
  //清空当前商品的购物车信息
  [CLEAR_CART](state, supplierid) {
    state.cartList[supplierid] = null;
    state.cartList = {
      ...state.cartList
    };
    setStore('buyCart', state.cartList);
